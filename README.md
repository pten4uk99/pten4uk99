...
#### 8. ГК Астра. Апрель 2023 - Настоящее время
- Стек: FastAPI, Django, virtualbox, postgresql, ReactJS.
- Полностью переписал один из сервисов, написанный на Django - на FastAPI + ReactJS.
Обосновал необходимость этого, самостоятельно спроектировал новую архитектуру и БД.
В результате сервис получил обновленный дизайн, unit тесты + End-To-End тесты на selenium.
Так как сервис использовал сложные sql запросы для своей работы, благодаря новой архитектуре БД
скорость выполнения задач сервиса возросла с 30-200 минут до нескольких секунд.
- Спроектировал систему сервисов, позволяющих автоматизировать некоторые 
процессы РБПО. Принимал участие как один из разработчиков в написании кода. 
Задеплоил разработанные сервисы.
- Изучил и активно применяю в создаваемых сервисах шаблоны проектирования.


## В процессе изучения
Прохожу курс повышения квалификации "Архитектор ПО". В свободное время смотрю уроки по 
машинному обучению (в будущем хотел бы активно этим заниматься).


## О себе
Более 3-х лет опыта разработки на языке Python. 
Обладаю неплохой степенью перфекционизма, люблю когда все разложено по полочкам, 
без помойки (в коде стараюсь следовать тому же принципу). 
Если какая то задача меня зажигает, то могу с головой в нее погрузиться, и работать 24/7,
пока не добьюсь результата, который мне нужен. 
